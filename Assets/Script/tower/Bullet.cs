...
public class Bullet : MonoBehaviour {
    ...
    public int Type, DetailType, Power;
    public float Range;
    ...
    void OnTriggerEnter(Collider other)
    {
        if (Type == 1 && other.transform.tag == "Enemy")
        {
            ...
            if (DetailType == 3 || DetailType == 5)
            {
                Collider[] colls = Physics.OverlapSphere(transform.position, Range); // 원점을 중심으로 일정 범위 안에 충돌 객체 찾음
                foreach (Collider coll in colls)
                {
                    ...
                    if(coll.GetComponent<Rigidbody>() != null)
                    {
                        ...
                        coll.GetComponent<Rigidbody>().AddExplosionForce(400.0f, transform.position, Range, 300.0f);
                    }
                }
            }
            else
        ...
        else if (Type == 3 && other.transform.tag == "Enemy")
        {
            ...
            if (DetailType == 1)
            {
                ...
                foreach (Collider coll in colls)
                    coll.GetComponent<info>().Debuff(Power); // 일정 범위 안의 충돌 객체들 일시적 Speed 감소
            }
            else
                other.GetComponent<info>().Debuff(Power);    // 충돌한 적 일시적 경직
            Destroy(gameObject);
        }
    }
}
